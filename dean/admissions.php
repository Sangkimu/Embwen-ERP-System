...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$admissionId || !in_array($decision, ['admitted', 'rejected'], true) || ($decision === 'admitted' && !in_array($residency, ['boarder', 'dayscholar'], true))) {
        ...
        try {
            ...
            if ($decision === 'admitted') {
                ...
                $courseDurationStmt = $pdo->prepare('SELECT duration_months FROM courses WHERE course_id=? LIMIT 1');
                $courseDurationStmt->execute([(int)$application['course_id']]);
                $durationMonths = (int)$courseDurationStmt->fetchColumn();
                $studyStartDate = date('Y-m-d');
                $studyEndDate = (new DateTime($studyStartDate))->modify('+' . $durationMonths . ' months')->format('Y-m-d');
                $studentStmt = $pdo->prepare("INSERT INTO students (id_no, name, course_id, residency, study_start_date, expected_end_date, status) VALUES (?, ?, ?, ?, ?, ?, 'active')");
                $studentStmt->execute([$admissionNumber, $application['applicant_name'], $application['course_id'], $residency, $studyStartDate, $studyEndDate]);
...
<div style="overflow-x:auto"><table class="application-table"><thead><tr><th>Applicant</th><th>Course</th><th>Application date</th><th>Status</th><th>Admission no.</th><th>Residency / Decision</th></tr></thead><tbody>
