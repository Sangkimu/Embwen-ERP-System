...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['action']) && $_POST['action'] === 'run_invoicing') {
    ...
    if (!empty($academicYear) && $semester) {
        try {
            ...
            $insertInvoiceStmt = $pdo->prepare("INSERT INTO fee_payments (student_id, fee_structure_id, amount_paid, payment_date, payment_method, receipt_no, reference_no, received_by) VALUES (?, ?, 0.00, CURDATE(), 'cash', ?, 'INVOICE_GEN', ?)");
