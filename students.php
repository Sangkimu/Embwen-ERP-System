...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!validPhoneNumber($_POST['phone'] ?? '') || !validPhoneNumber($_POST['parent_phone'] ?? '')) {
    ...
    $stmt=$pdo->prepare("INSERT INTO students(id_no,name,national_id,birth_certificate_no,phone,parent_name,parent_phone,previous_academic_level,year_of_completion,course_id,status) VALUES(?,?,?,?,?,?,?,?,?,?,?)");
    $stmt->execute([$idNo, trim($_POST['name']), $nationalId !== '' ? $nationalId : null, $birthCertificateNo !== '' ? $birthCertificateNo : null, trim($_POST['phone']), trim($_POST['parent_name']), trim($_POST['parent_phone']), trim($_POST['previous_academic_level']), $_POST['year_of_completion'] !== '' ? (int)$_POST['year_of_completion'] : null, $_POST['course_id'] !== '' ? $_POST['course_id'] : null, $_POST['status']]);
