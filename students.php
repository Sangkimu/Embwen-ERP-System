...
<div class="modal" id="studentModal"><div class="modal-box"><div class="modal-head"><h2>Add Student</h2><button type="button" onclick="closeModal()">×</button></div><form method="post"><label>Student ID Number<input name="id_no" placeholder="Leave blank to auto-generate"></label><label>Full Name <span style="color:#dc2626;">*</span><input name="name" required></label><label>Student Phone <span style="color:#dc2626;">*</span><input name="phone" type="tel" inputmode="numeric" pattern="\d{10}" maxlength="10" placeholder="10 digits e.g. 0712345678" required></label><label>Parent/Guardian Name <span style="color:#dc2626;">*</span><input name="parent_name" required></label><label>Parent/Guardian Phone <span style="color:#dc2626;">*</span><input name="parent_phone" type="tel" inputmode="numeric" pattern="\d{10}" maxlength="10" placeholder="10 digits e.g. 0712345678" required></label><label>Previous Academic Level<input name="previous_academic_level" placeholder="e.g. KCSE, Craft Certificate"></label><label>Year of Completion<input name="year_of_completion" type="number" min="1950" max="2100" placeholder="e.g. 2025"></label><label>Course <span style="color:#dc2626;">*</span><select name="course_id" required><option value="">Select course</option><?php foreach($courses as $c): ?><option value="<?= $c['course_id'] ?>"><?= htmlspecialchars($c['course_code'].' — '.$c['course_name']) ?></option><?php endforeach; ?></select></label><label>Status<select name="status"><option value="active">Active</option><option value="graduated">Graduated</option><option value="withdrawn">Withdrawn</option></select></label><div class="modal-actions"><button type="reset" class="clear-button">Clear</button><button type="button" class="secondary" onclick="closeModal()">Cancel</button><button type="submit" class="primary">Save Student</button></div></form></div></div><script src="assets/js/app.js"></script></body></html>