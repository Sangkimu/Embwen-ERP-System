...
</head><body><div class="app"><aside class="sidebar"><?php include "../partials/sidebar.php";?></aside><main class="main"><header class="topbar"><div class="module-tag">Workspace / <b>Student Portal</b></div><div class="top-user"><div class="avatar"><?=strtoupper(substr(user()['name'],0,2))?></div><?=htmlspecialchars(user()['name'])?></div></header><section class="content"><div class="page-head"><div><p class="eyebrow">STUDENT PORTAL — <?=htmlspecialchars($active_sem_name)?></p><h1>Welcome, <?=htmlspecialchars(user()['name'])?></h1><p>Access your student services.</p></div></div><div class="stats"><div class="stat"><span>Student ID</span><strong><?=htmlspecialchars($student['id_no']??'N/A')?></strong></div><div class="stat"><span>Department</span><strong><?=htmlspecialchars($student['department_name']??'Pending')?></strong></div><div class="stat"><span>Course period</span><strong><?=!empty($student['duration_months'])?htmlspecialchars($student['duration_months']).' months':'Pending'?></strong></div><div class="stat"><span>Course</span><strong><?=htmlspecialchars($student['course_code']??'N/A')?></strong></div><div class="stat"><span>Payment Total</span><strong><?=money($payments)?></strong></div><div class="stat"><span>Notices</span><strong><?=htmlspecialchars($notices)?></strong></div></div>
